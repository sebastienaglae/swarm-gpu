...
const scenarios = [
  { population: 100_000, workgroupSize: 128 },
  { population: 250_000, workgroupSize: 128 },
  { population: 500_000, workgroupSize: 128 },
  { population: 1_000_000, workgroupSize: 128 },
  { population: 500_000, workgroupSize: 256 },
];
const width = 1280;
const height = 720;
...
for (const scenario of scenarios) {
  ...
  const id = `SIM-${scenario.population}-WG${scenario.workgroupSize}-${width}X${height}`;
  const report = {
    schemaVersion: '1.0.0',
    scenario: {
      id,
      seed: 0x5a17c9e3,
      instanceCount: scenario.population,
      canvasWidth: width,
      canvasHeight: height,
      fixedTimestepSeconds: 1 / 60,
      workgroupSize: scenario.workgroupSize,
    },
    ...
    measurement: {
      warmupSeconds,
      sampleSeconds,
      frameSampleCount: frameSamples.frameIntervalMs.length,
      gpuSampleCount,
    ...
    results: {
      ...
      gpuComputeMs: percentiles(gpuSamples.map((sample) => sample.computeMs)),
      gpuRenderMs: percentiles(gpuSamples.map((sample) => sample.renderMs)),
      gpuTotalMs: percentiles(gpuSamples.map((sample) => sample.totalMs)),
      longFrameCount: frameSamples.frameIntervalMs.filter((value) => value > 33.33).length,
      drawCalls: 2,
      computeDispatches: 1,
      readbacksPerInteractiveFrame: 0,
      estimatedExplicitGpuBytes: scenario.population * 80 + width * height * 4 + 520,
    },
  };
  ...
  process.stdout.write(
    `${id}: frame ${String(report.results.frameIntervalMs.median)} ms, compute ${String(report.results.gpuComputeMs.median)} ms, render ${String(report.results.gpuRenderMs.median)} ms\n`,
  );
  await page.close();
}
await browser.close();
